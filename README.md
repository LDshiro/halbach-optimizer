...
このリポジトリの公式なローカル開発ターゲットは Python 3.14 です。
...
- `summary.json`, `near_graph.json`, `solver_trace.csv`, `check_report.json` など
...
halbach/
  ...
  geom.py                   # ROI/幾何ヘルパ
  types.py                  # Geometry/型定義
  near.py                   # 近傍グラフと近傍インデックス
  symmetry.py               # x=0 対称処理
  io.py                     # run 入出力補助
  robust.py                 # ロバスト目的（gradnorm など）
  magnetization_runtime.py  # self-consistent 磁化の実行時構築
  sc_debug.py               # self-consistent デバッグ bundle
  sc_linear_system.py       # self-consistent 近傍の線形化
  demag_cellavg.py          # セル平均 demag tensor（N）
  numba_compat.py           # Numba 互換 (TYPE_CHECKING)
  ...
  autodiff/
    ...
    jax_demag_cellavg.py                    # JAX版 demag tensor
  cli/
    generate_run.py
    optimize_run.py
  gui/
