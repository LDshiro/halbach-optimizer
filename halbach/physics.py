...
@njit(cache=True)
def compute_B_and_B0_phi_rkn_masked(
    phi_rkn: NDArray[np.float64],
    ring_active_mask: NDArray[np.bool_],
    r_bases: NDArray[np.float64],
    cth: NDArray[np.float64],
...
@njit(cache=True)
def objective_only_phi_rkn_masked(
    phi_rkn: NDArray[np.float64],
    ring_active_mask: NDArray[np.bool_],
    r_bases: NDArray[np.float64],
    cth: NDArray[np.float64],
    ...
    Bx, By, Bz, B0x, B0y, B0z = compute_B_and_B0_phi_rkn_masked(
        phi_rkn,
        ring_active_mask,
        r_bases,
        cth,
