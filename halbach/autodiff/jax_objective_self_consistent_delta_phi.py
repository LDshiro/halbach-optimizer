...
_T = TypeVar("_T")
_MIRROR_CACHE: dict[int, NDArray[np.float64]] = {}
...
def _get_mirror_basis(N: int) -> NDArray[np.float64]:
    basis = _MIRROR_CACHE.get(N)
    if basis is None:
        basis = build_mirror_x0(N).basis
        _MIRROR_CACHE[N] = basis
    return basis
...
def objective_with_grads_self_consistent_delta_phi_x0_jax(
    ...
    *,
    ...
    """
    JAX objective and gradients for x=0 mirror delta-phi model (self-consistent).
    ...
    basis = _get_mirror_basis(int(geom.theta.shape[0]))
    ...
    if use_jit:
        ...
        (J, aux), grads = vg(delta_rep_j, r_bases_j)
    else:
...
__all__ = ["objective_with_grads_self_consistent_delta_phi_x0_jax"]
