...
from typing import TYPE_CHECKING, Any, Literal, TypeAlias, TypedDict, TypeVar, cast
...
if TYPE_CHECKING:
    # mypy用：njitを「関数シグネチャを保つデコレータ」として扱う
...
def objective_with_grads_fixed(
    ...
    DBy = By - B0y
    DBz = Bz - B0z
    J = cast(float, np.mean(DBx * DBx + DBy * DBy + DBz * DBz))
    sumDBx = np.sum(DBx)
    sumDBy = np.sum(DBy)
    sumDBz = np.sum(DBz)
