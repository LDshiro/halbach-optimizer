...
def test_run_perturbation_case_smoke(tmp_path: Path) -> None:
    ...
    assert result.B_roi.shape == (cfg.roi_samples, 3)
    assert result.pts_roi.shape == (cfg.roi_samples, 3)
    assert result.ppm_roi.shape == (cfg.roi_samples,)
    assert result.map_xy.ppm.shape == (len(result.map_xy.ys), len(result.map_xy.xs))
    assert np.isfinite(result.ppm_roi).all()
    assert np.isfinite(result.map_xy.ppm[result.map_xy.mask]).all()
